...
var puBlic={
    ...
    init: function() {
        ...
        this.initParams();
        this.load();
        this.getCaseList(this.params.menuId, this.params.menuIndex)
    },
    initDom: function() {
        var dom = this.dom;
    ...
    bindEvent:function () {
        ...
        dom.works_list.on('click','li',function () {
            params.curPageNum = 1
            params.menuId = $(this).attr('data-curid')
            params.menuIndex = $(this).attr('data-index')
            $(this).addClass('active').siblings().removeClass('active')
            puBlic.getCaseList(params.menuId,params.menuIndex)
        })

        //work todo ul
        dom.works_todo.on('click','li',function (e) {
            $(this).addClass('active').siblings().removeClass('active')
            params.arrIndex = $(this).attr('data-index')
            puBlic.getWorkCase(params.arrList[params.arrIndex])
        })
    ...
    allPaging : function (callback) {
        ...
        $('.tabs-up').click(function () {
            if(params.arrIndex == 0) {
                alert('已经是第一个了')
                return false
            } else {
                params.arrIndex--
                $("#workTodo li").eq(params.arrIndex).addClass('active').siblings().removeClass('active')
                puBlic.getWorkCase(params.arrList[params.arrIndex])
            }
        })
        $('.tabs-down').click(function () {
            if(params.arrIndex == params.arrList.length - 1) {
                alert('已经是最后一个了')
                return false
            } else {
                params.arrIndex++
    ...
    // work case
    getWorkCase : function (data) {
        $("#worksImg").html('')
        const urlList = data.img_arr.split(',')
        urlList.forEach(urlItem => {
            urlItem = urlItem.replace(/\s*/g,"");
            $("#worksImg").append(`<img src="/Uploads/${urlItem}" class="works-img" title="${data.title}" alt="${data.title}">`)
        })
        $('.scroll-active').css('top','0%')
    },
    getCaseList : function(fieldId, menuIndex){
        console.log('shiu', fieldId, menuIndex)
        $("#workTodo").html('')
        $(".works-list").html('')
        $.ajax({
            type:'GET',
            url:'/index.php/Work/info',
            data: { id: fieldId },
            response: JSON,
            success:function(res){
                const returnRes =eval(JSON.parse(res))
                if(returnRes.code == 200) {
                    const { data } =  returnRes
                    puBlic.params.arrList = data.list;
                    data.arr.forEach((menuItem,index) => {
                        $(".works-list").append(`<li class="wow fadeInLeft  ${ index == menuIndex ? 'active':''}"  data-wow-delay="0.4s" data-index="${index}" data-curid="${ menuItem.menu.id }">${ menuItem.menu.title }</li>`)
                    })
                    if(data.list && data.list.length) {
                        console.log('类别名称', data.list)
                        data.list.forEach((listItem,index) => {
                            $("#workTodo").append(`<li class="wow fadeInLeft  ${ index == 0 ? 'active':''}"" data-wow-delay="0.4s" data-index="${index}" data-curid="${listItem.id}">${listItem.title}</li>`)

                            if(index == 0 && listItem.img_arr) {
                                puBlic.getWorkCase(listItem)
                            }
                        })
                    }
