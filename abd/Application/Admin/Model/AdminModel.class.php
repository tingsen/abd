...
class AdminModel extends Model {
    ...
    public function login($account = '', $password = '') {
        ...
        if($admin){
            //修改最后登录时间和ip
            $this->where(array('a_id'=>$admin['a_id']))->data(array('last_login_time'=>time(),'last_login_ip'=>$_SERVER["REMOTE_ADDR"]))->save();
            ///设置session
            session('A_ACCOUNT',$admin['account']);
            session('A_ID',$admin['a_id']);
            $admin['group_id'] != 0 ? session('A_GROUP',$admin['group_id']) : '';
            return true;
        }else{
            return false;
        }
